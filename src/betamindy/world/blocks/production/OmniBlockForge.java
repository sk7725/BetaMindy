...
/** Copy of the 6.0 BlockForge class.
 * @author Anuke */

//TODO use when 7.0 comes out!

public class OmniBlockForge extends PayloadAcceptor{
    public float buildSpeed = 0.4f;
    public int minBlockSize = 1, maxBlockSize = 2;

    public OmniBlockForge(String name){
        super(name);
        ...
        config(Block.class, (OmniBlockForgeBuild tile, Block block) -> {
            if(tile.recipe != block) tile.progress = 0f;
            tile.recipe = block;
        });

        consumes.add(new ConsumeItemDynamic((OmniBlockForgeBuild e) -> e.recipe != null ? e.recipe.requirements : ItemStack.empty));
    }
    ...
    public void setBars(){
        ...
        bars.add("progress", (OmniBlockForgeBuild entity) -> new Bar("bar.progress", Pal.ammo, () -> entity.recipe == null ? 0f : (entity.progress / entity.recipe.buildCost)));
    }
    ...
    public class OmniBlockForgeBuild extends PayloadAcceptorBuild<BuildPayload>{
