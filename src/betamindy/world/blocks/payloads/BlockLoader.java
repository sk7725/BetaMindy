    ...
    public class BlockLoaderBuild extends PayloadBlockBuild<BuildPayload>{
        ...
        public boolean acceptPayload(Building source, Payload payload){
            return super.acceptPayload(source, payload) &&
                ...
                ((((BuildPayload)payload).build.block.hasItems && ((BuildPayload)payload).block().unloadable
                && ((BuildPayload)payload).block().itemCapacity >= 10 && ((BuildPayload)payload).block().size <= maxBlockSize)/* ||
